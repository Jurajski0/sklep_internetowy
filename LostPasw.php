...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (empty($email)) {
        ...
        $stmt = $pdo->prepare("SELECT reset_token_expires FROM users WHERE email = :email");
        $stmt->bindParam(':email', $email);
        $stmt->execute();
        $reset_token_expires = $stmt->fetchColumn();

        $now = new DateTime();
        if ($reset_token_expires && $now < new DateTime($reset_token_expires)) {
...
function sendPassReset($email, $token) {
    ...
    $resetLink = "https://xxx/reset_password.php?token=" . urlencode($token);
    $message = "Kliknij poniższy link, aby zresetować hasło (ważny przez 1 godzinę): \n\n" . $resetLink;
    $headers = "From: no-reply@Kep&Zoś.pl\r\n";
